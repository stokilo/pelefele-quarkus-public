                ...
                <div class="columns">
                  <div class="column">
                    ...
                    <ValidationProvider v-slot="{ valid, failedRules }" rules="required|min:5|max:250" vid="location">
                      ...
                      >
                        <b-autocomplete
                          v-model="listingStore.listing.location.location"
                          field="location"
                  ...
                  <div class="column is-one-third">
                    <div class="columns">
                      <div class="column is-half">
                        <b-field label="Type">
                          <b-select v-model="listingStore.listing.propertyType" placeholder="Select a proprety type">
                            <option
                              v-for="option in listingStore.listingPropertyTypes"
                              :key="option.id"
                              :value="option.id"
                            >
                              {{ option.value }}
                      ...
                      <div class="column is-half">
                        <b-field label="Target">
                          <b-select v-model="listingStore.listing.purpose" placeholder="Select a purpose">
                            <option
                              v-for="option in listingStore.listingPurposes"
                              :key="option.id"
                              :value="option.id"
                            >
                              {{ option.value }}
...
import { LocationDTO } from '~/store/generated'
...
export default class CreateListing extends Vue {
  ...
  async onSaveListing () {
    ...
    try {
      const apiResponse = await this.listingStore.onSaveListing()
      this.$log.info(apiResponse)
  ...
  async onLocationSelect (selectedLocation: LocationDTO) {
    if (selectedLocation) {
